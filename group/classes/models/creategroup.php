            ...
            foreach($list as $l){
                ...
                foreach($user as $u){
                    ...
                    foreach($courses as $c){
                        ...
                        echo "usuarios".$users->id."\n"; 
                        $course = $DB->get_record("course", array("shortname" => $c));
                        echo "curso".$course->id."\n";
                        if($users){
                            if($course){
                                $existUsers = $DB->get_record_sql(
                                    "SELECT mu.id as userid, mc.id as courseid
                                        from mdl_user mu 
                                        ...
                                        inner join mdl_course mc on mc.id = me.courseid 
                                            WHERE mu.id = $users->id and mc.id = $course->id"
                                );

                                if($existUsers){
                                    $group = $DB->get_record("groups", array('name' => $l->fields->Name, 'courseid' => $existUsers->courseid));
                                    $group_exist_user = $DB->get_record("groups_members", array('groupid' => $group->id, 'userid' => $users->id)); 
                                    if(!$group_exist_user){
